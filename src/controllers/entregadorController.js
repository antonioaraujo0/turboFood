...
const listar = async (req, res) => {
  try {
      ...
      include: [
        {
          model: Veiculo,
          as: "veiculo",
          attributes: ["id", "modelo", "placa", "tipo"],
        },
      ],
    });
    ...
    res
      .status(500)
      .json({ erro: "Erro ao listar entregadores", detalhe: error.message });
  }
};
...
const buscarPorId = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ erro: "Erro ao buscar entregador", detalhe: error.message });
  }
};
...
const criar = async (req, res) => {
  try {
    const { nomeCompleto, email, telefone, cpf, cnh, tipoVeiculo, senha } =
      req.body;
    ...
    if (cnhExistente)
      return res.status(409).json({ erro: "CNH já cadastrada" });
    ...
    res
      .status(500)
      .json({ erro: "Erro ao criar entregador", detalhe: error.message });
  }
};
...
const atualizar = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ erro: "Erro ao atualizar entregador", detalhe: error.message });
  }
};
  ...
  try {
    ...
    if (entregador.status === "em_entrega") {
      return res
        .status(409)
        .json({
          erro: "Não é possível remover um entregador que está em entrega",
        });
    }
    ...
    res
      .status(500)
      .json({ erro: "Erro ao remover entregador", detalhe: error.message });
  }
};
